...
- 任务看板升级为 Notion-lite 查询视图：搜索支持多关键词 AND 与引号短语，并可按状态、所属待办纸、创建/完成日期、计划日期范围和备注存在性组合筛选；表格新增计划开始日与截止日列，支持有优先级的多级排序和点击列头置顶规则。视图、筛选和排序会随重启保留，临时搜索词不保存。搜索与筛选同时作用于活动月历；月历以跨日连续条展示创建到完成（未完成到今天）的历史跨度，空间不足时可从准确的“还有 N 项”浮层访问当天全部任务并导航回原待办纸。
