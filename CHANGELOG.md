...
- 任务看板表格升级为 Notion-lite 查询视图：搜索支持多关键词 AND 与引号短语，并可按状态、所属待办纸、创建/完成日期、计划日期范围和备注存在性组合筛选；表格新增计划开始日与截止日列，支持有优先级的多级排序和点击列头置顶规则。视图、筛选和排序会随重启保留，临时搜索词不保存；搜索与筛选同时作用于现有月历。
