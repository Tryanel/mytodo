...
- 空白待办行不再提前记录任务创建时间；任务会在首次填写正文、备注、提醒或关联内容时记录创建时刻，之后编辑、完成、恢复和计划调整都不会重置。升级旧数据时，缺少创建时间的既有任务会使用首次迁移时刻近似补齐。
- 在待办纸直接勾选完成单条任务后，原任务附近会短暂出现不抢焦点的“记录完成过程”入口；只有点击后才打开备注编辑器，批量完成和插件/MCP 自动完成不会打扰桌面操作。
- 任务备注编辑器现在会在原待办纸折叠或隐藏后继续保留并标识所属任务；切换到另一任务、关闭窗口、按 Esc、删除任务/纸片或退出应用遇到未保存草稿时，会在编辑器内明确询问保存、放弃或取消。退出时任一取消都会保留所有草稿并停止退出；保存后再删除可通过一次撤销恢复完整备注。保存仍写回原任务，不会强行展开隐藏纸片或抢走其他窗口焦点；若 MCP、插件或其他操作先删除原对象，编辑器会明确标记草稿已失效，避免写到错误任务。
- 待办任务现在可以设置可选的计划开始日和截止日；任一日期都可单独使用，也可设置合法范围。计划日期与实际创建、完成时间相互独立，会随保存、重启和撤销/重做保留，并可从原待办纸、MCP 或新的插件协议 1.9 修改或清除；宿主继续兼容不使用新合同成员的 1.8 插件。Todo 与 Board 的 Markdown 全量导出也会为每条任务写出两端计划日期。
- 任务看板升级为 Notion-lite 查询视图：搜索支持多关键词 AND 与引号短语，并可按状态、所属待办纸、创建/完成日期、计划日期范围和备注存在性组合筛选；表格新增计划开始日与截止日列，支持有优先级的多级排序和点击列头置顶规则。视图、筛选和排序会随重启保留，临时搜索词不保存。搜索与筛选同时作用于活动月历和计划时间线：活动月历以连续条展示创建到完成（未完成到今天）的历史跨度，空间不足时可从准确的“还有 N 项”浮层访问当天全部任务；计划时间线支持周/月窗口，以连续条或单日标记展示计划日期，并保留可导航的未排期任务列表。所有 Board 条目仍只导航回原待办纸。
- 插件协议升级到 1.10：插件可显式提供当前完整正文的 Markdown 导出；只有声明并实现该能力的插件纸显示导出入口。导出直接读取当前会话，不再把胶囊摘要或旧缓存误当正文；失败或会话失效时不会创建文件。旧 1.8/1.9 插件仍可继续加载。

### v4.0.1-preview

- 修复任务看板正文覆盖纸片底部圆角的问题，使看板纸与其他纸张外形一致。
- 任务备注编辑器改为非模态窗口；编辑期间仍可使用边缘收起按钮和任务胶囊，切换到其他窗口时也不会触发原任务纸自动收起。

### v4.0.0-preview

**待办记录、导出与任务看板**

- 每条待办现在可以添加独立的多行备注，并自动记录创建时间；勾选完成时记录完成时间，恢复为未完成时清除完成时间。旧数据会在首次加载时安全补齐缺失时间，备注与时间也会随插件和 MCP 的待办读写接口传递。

- 每张纸的右键菜单新增“导出为 Markdown”：待办纸会导出全部任务的勾选状态、备注、创建时间和完成时间，笔记纸会导出当前正文。

- 托盘顶部新增全局单例的任务看板纸：它沿用普通纸片的移动、缩放、折叠胶囊、主题和 Markdown 导出能力，并以 Notion 数据库风格的表格查看全部任务、备注、所属纸片与时间。表格支持即时搜索、排序菜单和点击列头双向排序，排序偏好会自动保存；月历可查看从创建到完成（未完成则到今天）的跨度。点击表格行或日历任务可直接打开并定位到原纸片。
...
- 修复 PaperTodo 曾由其他 Windows 账户或受限运行身份启动时，当前用户双击程序只会静默退出、无法唤醒自己的实例的问题；单实例通信现在按 Windows 用户隔离。
